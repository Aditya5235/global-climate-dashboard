...
def co2_emission(country_code , indicator ,start , end):
...
# | ``EN.GHG.CO2.PC.CE.AR5`` | CO₂ emissions per capita (excluding LULUCF) | t CO₂e per person |
    ...
    url = (f'https://api.worldbank.org/v2/country/{country_code}/indicator/{indicator}?format=json&per_page=500')
    meta_url = (f'https://api.worldbank.org/v2/indicator/{indicator}?format=json')
